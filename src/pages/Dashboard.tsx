        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-initial">
              ...
              <input
                className="pl-9 pr-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-48"
        ...
        {showForm && (
          <div className="mb-6 p-4 border rounded-lg bg-gray-50">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              <input className="px-3 py-2 border rounded text-sm" placeholder={t.inventory.name} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              <input className="px-3 py-2 border rounded text-sm" placeholder={t.inventory.sku} value={form.sku} onChange={e => setForm(f => ({ ...f, sku: e.target.value }))} />
              <input className="px-3 py-2 border rounded text-sm" placeholder={t.inventory.category} value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} />
              <input className="px-3 py-2 border rounded text-sm" placeholder={t.inventory.price} type="number" step="0.01" value={form.price} onChange={e => setForm(f => ({ ...f, price: parseFloat(e.target.value) || 0 }))} />
              <input className="px-3 py-2 border rounded text-sm" placeholder={t.inventory.quantity} type="number" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: parseInt(e.target.value) || 0 }))} />
              <input className="px-3 py-2 border rounded text-sm" placeholder={t.inventory.description} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            </div>
